...
//            EDACCApp.getApplication().show(removeInstances);
     ...
     * @throws SQLException
     */
    public Vector<Instance> buildInstancesAutogenerateClass(Vector<File> instanceFiles, File ret, Tasks task, boolean compressBinary) throws FileNotFoundException, IOException, NoSuchAlgorithmException, NullPointerException, SQLException, InstanceClassAlreadyInDBException {
        ...
        for (int i = 0; i < instanceFiles.size(); i++) {
            try {
                ...
                try {
                    ...
                    if (nodes == null) {
                        nodes = new DefaultMutableTreeNode(InstanceClassDAO.createInstanceClass(possibleInstanceClasses[0], "Autogenerated instance source class", null, true));
                    }
    ...
    private InstanceClass getInstanceClassFromTree(String[] possibleInstanceClasses, DefaultMutableTreeNode node, int i) throws SQLException, InstanceClassAlreadyInDBException {
        ...
        if (i == possibleInstanceClasses.length - 1) {
            ...
            DefaultMutableTreeNode tmp = new DefaultMutableTreeNode(InstanceClassDAO.createInstanceClass(possibleInstanceClasses[(i)], "Autogenerated instance source class", (InstanceClass) node.getUserObject(), true));
            node.add(tmp);
                ...
                if (input != null) {
                    if (input.isSource()) {
                        ...
                        for (int i = 0; i < selectedRows.length; i++) {
                            ...
                            toChange.get(i).setFile(null);
                            InstanceDAO.save(toChange.get(i), false);
                        }
                    } else {
                        for (int i = 0; i < selectedRows.length; i++) {
