...
public class ParserController implements Runnable{
    ...
    public ParserController(Experiment exp, Vector<SolverProperty> givenProperties, boolean recompute) throws NoConnectionToDBException, SQLException, IOException, SolverPropertyTypeNotExistException, SolverPropertyNotInDBException{
    ...
    public void run() {
        for(int i = 0; i < availableProcessors; i++){
            ...
            if(queue.isEmpty())
                break;
            new Thread(new ResultPropertyParser(queue.remove(0), this));
            lock.unlock();
        }
    }

    private void createParserJobs() throws NoConnectionToDBException, SQLException, IOException, SolverPropertyTypeNotExistException, SolverPropertyNotInDBException {
